...
def test_archive_organisation_prompts_user(client_request, platform_admin_user, organisation_one, mocker):
    ...
    assert normalize_spaces(delete_page.select_one('.banner-dangerous').text) == (
        'Are you sure you want to delete ‘organisation one’? '
...
def test_archive_organisation_after_confirmation(
    ...
    # First user removed from the cache is an invited user, others are current org users
    assert redis_delete_mock.call_args_list == [
        mocker.call('user-73616d70-6c65-4f6f-b267-5f696e766974', 'user-1234', 'user-5678'),
        mocker.call('organisations'),
        mocker.call('live-service-and-organisation-counts'),
    ]


@pytest.mark.parametrize('api_error_message, displayed_message', [
    ('Organisation has services', 'Can’t archive an organisation with services'),
    ('Organisation has team members', 'Can’t archive an organisation with team members'),
])
def test_archive_organisation_does_not_allow_orgs_with_team_members_or_services_to_be_archived(
    ...
    mocker.patch('app.organisations_client.archive_organisation', side_effect=HTTPError(
        response=mocker.Mock(
            status_code=400,
            json={'result': 'error', 'message': api_error_message}
        ),
    ...
    assert page.find("div", class_="banner-dangerous").text.strip() == displayed_message
